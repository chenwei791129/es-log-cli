...
benchmark 顯示：缺少 server-side 聚合是 es-log 無法取代既有 `_msearch` 聚合查詢路徑的唯一阻礙。常見的 aggregation-heavy 查詢（`sum(bytes)` + `date_histogram` 時間分桶、Top-N buckets、unique-value 計數）目前都得拉數百萬筆 raw doc 在 client 端聚合，不可行。
...
- 手動驗證：`es-log -c prod search -t netflow --since 1h --aggs '{"top_isp":{"terms":{"field":"isp","size":10,"order":{"bytes":"desc"}},"aggs":{"bytes":{"sum":{"field":"bytes"}}}}}'` 回傳 Top ISP 桶。
