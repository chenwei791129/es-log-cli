...
Define the GitHub Actions automation that gates merges into `main` and publishes
releases for `es-log`: running the Go test suite and cross-platform build checks
on pull requests, driving release-please on `main`, and building versioned
cross-platform binaries as downloadable release assets.
