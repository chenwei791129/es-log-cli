...
When an aggregation response reports `_shards.failed` greater than zero, the command SHALL print a warning to stderr listing the failure reason and SHALL still emit the aggregation results that were returned. The command SHALL NOT report empty or zero aggregation results silently when shards failed.

#### Scenario: fielddata shard failure is surfaced not silenced

- **WHEN** an aggregation request returns HTTP 200 with `_shards.failed` greater than zero (e.g. `Fielddata is disabled`)
- **THEN** the command SHALL print a warning to stderr with the failure reason and emit the partial aggregations rather than reporting zero
